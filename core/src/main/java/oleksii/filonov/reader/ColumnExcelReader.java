...
public class ColumnExcelReader implements ColumnReader {
	...
	public String[] getUniqueColumnValues(final Sheet sheetToRead, final String columnMarker) {
		...
		final int columnIndex = columnReaderHelper.findColumnIndex(rows, columnMarker);
		while (rows.hasNext()) {
			final Row row = rows.next();
			final Cell cell = row.getCell(columnIndex);
			if (columnReaderHelper.isStringType(cell) && !result.contains(cell.getStringCellValue())) {
