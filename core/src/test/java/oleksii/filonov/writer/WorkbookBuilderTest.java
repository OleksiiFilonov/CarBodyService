...
public class WorkbookBuilderTest {
    ...
    private static final String VIN_LIST_1 = "Link1";
    private static final String VIN_LIST_2 = "Link2";
    private static final String VIN_LIST_3 = "Link3";
    private static final String VIN_LIST_4 = "Link4";
	...
	public void formResultFileWithLinks() throws IOException, InvalidFormatException {
        ...
        bodyIdLinks.put(BODY_ID_FOUND, VIN_LIST_1);
        bodyIdLinks.put(BODY_ID_FOUND, VIN_LIST_2);
        bodyIdLinks.put(ANOTHER_BODY_ID_FOUND, VIN_LIST_3);
        bodyIdLinks.put(BODY_ID_FOUND, VIN_LIST_4);
		excelBuilder.assignTasks(bodyIds, bodyIdLinks);
        verifyCreatedDocument();
	}

    private void verifyCreatedDocument() throws IOException {
        verify(firstLinkToVinCell).setCellValue(VIN_LIST_1);
        verify(firstHyperLink).setAddress(PATH_TO_CAMPAIGN_FILE + "#" + VIN_LIST_1);
        verify(clientSheet).shiftRows(eq(FOUND_ROW_INDEX + 1), anyInt(), eq(SHIFT_VALUE));
        verify(secondLinkToVin).setCellValue(VIN_LIST_2);
        verify(secondHyperLink).setAddress(PATH_TO_CAMPAIGN_FILE + "#" + VIN_LIST_2);
        verify(clientSheet).shiftRows(eq(FOUND_ROW_INDEX + 2), anyInt(), eq(SHIFT_VALUE));
        verify(fourthLinkToVin).setCellValue(VIN_LIST_4);
        verify(fourthHyperLink).setAddress(PATH_TO_CAMPAIGN_FILE + "#" + VIN_LIST_4);
        verify(notFoundBodyIdCell, never()).getRow();
        verify(linkToAnotherVinCell).setCellValue(VIN_LIST_3);
        verify(thirdHyperLink).setAddress(PATH_TO_CAMPAIGN_FILE + "#" + VIN_LIST_3);
