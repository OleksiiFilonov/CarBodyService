...
public class XSSFBuilderTest {
    ...
    @Before
    public void setUp() {
        this.excelBuilder.createDocument();
        this.excelBuilder.createLinkedSheetWithName(LINKED_SHEET_NAME);
        this.excelBuilder.setColumnReaderHelper(this.columnReaderHelper);
    }

    @Test
    public void fillResultFileWithBodyId() throws IOException {
        this.excelBuilder.writeBodyIdsColumnToLinkedSheet(BODY_ID_MARKER, BODY_IDS);
        this.excelBuilder.saveToFile(Paths.get("", RESULT_FILE).toFile());
        final Map<String, Cell> bodyIdCellMap = this.excelBuilder.getBodyIdCellMap();
    ...
    public void formResultFileWitLinks() throws IOException, InvalidFormatException {
        this.excelBuilder.writeBodyIdsColumnToLinkedSheet(BODY_ID_MARKER,
                new String[] { REAL_BODY_ID_FIRST_SHEET_ROW_ONE, REAL_BODY_ID_FIRST_SHEET_ROW_TEN,
                        NO_SUCH_BODY_ID_FIRST_SHEET_ROW_TEN });
        this.excelBuilder.linkExistingBodyIds(VIN_MARKER, Paths.get("", COMPAIGN_FILE).toFile());
        this.excelBuilder.saveToFile(Paths.get("", LINKED_RESULT_FILE).toFile());
        final Map<String, Cell> bodyIdCellMap = this.excelBuilder.getBodyIdCellMap();
        final Cell linkedBodyIdCell = bodyIdCellMap.get(REAL_BODY_ID_FIRST_SHEET_ROW_ONE);
        assertEquals(REAL_BODY_ID_FIRST_SHEET_ROW_ONE, linkedBodyIdCell.getStringCellValue());
    ...
    private void checkWrittenLinkedResultFile() throws IOException, InvalidFormatException {
        ...
        final int bodyIdColumnIndex = this.columnReaderHelper.findColumnIndex(rows, BODY_ID_MARKER);
